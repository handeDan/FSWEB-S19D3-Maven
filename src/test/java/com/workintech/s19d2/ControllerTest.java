...
class ControllerTest {
  ...
  void saveAccount() throws Exception {
    ...
    mockMvc
        .perform(
            post("/account").contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(account)))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON))
  ...
  void accessSecuredEndpointsWithImproperRoleShouldFail() throws Exception {
    mockMvc.perform(post("/account")).andExpect(status().isForbidden());
  }
}
